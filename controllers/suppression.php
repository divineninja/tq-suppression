...
<?php
class Suppression extends Controller
{
    ...
    public function index()
    {
        ...
        $this->view->render('suppression/suppression', false, true);
    }

    public function get_suppression($params)
    {
        $param = json_decode(base64_decode($params));

        $this->model->find_suppression($param);
    }
    ...
    {
        $this->model->delete('question_suppression', "id = '{$id}'");
    }

    public function get_suppression_list($campaign)
    {
        ...
        $this->view->render('suppression/ajax/list', true);
    }
    ...
    public function manage_suppression()
    {
        $this->view->q_suppression = $this->model->select('SELECT * FROM `question_suppression`');
        $this->view->render('suppression/manage', false, true);
    }
    ...
    public function saveSupression()
    {
        ...
        $destination = dirname(dirname(__FILE__)).'/uploads/suppression/'.$file_destination;
        ...
        //execute save group
        $group_id = $this->model->save_suppression_group($group_data);
        ...
        // process upload file
        $url = $this->view->set_url("suppression/insertSupression/$supressionURI&id=$question_id&campaign=$campaign&group_id=$group_id");

        // execute using the url
        header("location: $url");
    }

    public function question_suppression()
    {
        foreach ($_POST['suppression'] as $key => $value) {
            $q_suppresion = array(
                    'campaign' => $_POST['campaign'],
                    'question' => $_POST['question'],
                    'group'    => $value
                );
            $this->model->insert('question_suppression', $q_suppresion);
        }
        $url = $this->view->set_url("suppression/assign");
        header("location: $url");
    }
    ...
    {
        $this->view->suppression_file = $this->model->suppressionFile();
        $this->view->campaigns = $this->get_campaigns();
        $this->view->render('suppression/assign', false, true);
    }
    ...
    public function truncate($id)
    {
        ...
        header("location: ".$this->view->set_url('suppression'));
    }
    ...
    public function insertSupression($supressionDetails)
    {
        ...
        $count_stat = $this->model->update('suppression_group', $count_sum, "id = $group_id");
        ...
        for ($i=1; $i <= $pages; $i++) {
            ...
            $insert_query = "INSERT INTO suppression(phone, `group`)VALUES $value_statement";
    ...
    public function find()
    {
        ...
        $this->view->render('suppression/find', false, true);
    }
    ...
    public function files_with_error()
    {
        ...
        $this->view->render('suppression/files_with_error', false, true);
    }
